...
class JurusanController extends SecureController{
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("jurusan.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Jurusan";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		...
		return $this->render_view("jurusan/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		...
		$page_title = $this->view->page_title = "Add New Jurusan";
	...
	function delete($rec_id = null){
		...
		$db->where("jurusan.id", $arr_rec_id, "in");
